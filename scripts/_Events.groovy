...
eventCompileSourcesStart = {
    if(compilingPlugin('guice')) return
    if(!buildConfig.griffon.compiler.additional.sources) buildConfig.griffon.compiler.additional.sources = []
    buildConfig.griffon.compiler.additional.sources << "${basedir}/src/guice"
}

eventStatsStart = { pathToInfo ->
    if(!pathToInfo.find{ it.path == 'src.guice'} ) {
        pathToInfo << [name: 'Guice Config', path: 'src.guice', filetype: ['.groovy', '.java']]
    }
}